...
export function QuickCapture({ open, onOpenChange }: QuickCaptureProps) {
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-overlay bg-ink-950/70 data-[state=open]:animate-in data-[state=closed]:animate-out" />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-dialog w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-overlay p-6 shadow-xl data-[state=open]:animate-in data-[state=closed]:animate-out">
          ...
          {/* Form */}
          <form
            // Server Action integration comes later
            className="space-y-4"
